# EC 441 – Intro to Computer Networking (Spring 2026)

## Repository Overview

This repository contains my weekly assignments and final project for EC 441. Each submission is an artifact demonstrating engagement with networking concepts, covering topics across all five layers of the network stack.

## Generative AI Usage

I plan to use **Claude (Anthropic)** as my primary generative AI tool for brainstorming and tailoring my reports. I will update this section as the semester progresses with details on how these tools were actually used.

## Repository Structure

Submissions are organized by week and topic:

```
EC441/
├── README.md
├── week01-physical-layer/
├── week02-.../
├── ...
└── final_project/
```

## Breadth Tracking

**Topics covered:** *(will be updated as work is submitted)*

- [ ] Information: sources, representation
- [x] Physical layer: media, propagation, signals, noise, data rates
- [ ] Link layer: frames, error control, CRC
- [ ] Multiple access: ALOHA, CSMA
- [ ] Ethernet: addressing, switching, ARP
- [ ] Wireless networks: 802.11
- [ ] Reliable data transfer: Stop-and-Wait, sliding window (GBN, SR)
- [ ] Network layer: forwarding, routing, IP addressing, CIDR, subnetting
- [ ] Routing: Link State (Dijkstra), Distance Vector
- [ ] Autonomous systems, BGP
- [ ] IPv4, IPv6, DHCP, NAT
- [ ] TCP: sequencing, connection management, congestion/flow control
- [ ] Tools: ping, traceroute, Wireshark, Mininet, sockets
- [ ] Applications: client/server, P2P
- [ ] Web: HTTP, HTML
- [ ] Security: cryptography, protocols

**Layers covered:**

- [x] Physical
- [ ] Data Link
- [ ] Network
- [ ] Transport
- [ ] Application

**Types included:**

- [x] Problem
- [ ] Lab
- [ ] Report
